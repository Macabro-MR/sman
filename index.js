...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./funçoes/lib/exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./funçoes/lib/myfunc')

//CONFIGURAÇÃO DA DATABASE
const { conectar_db } = require('./funçoes/connect');
const { 
pegar_apikey, 
Totalregistrados,
resetar_todos_limit
...
try {
  ...
  low = require('./funçoes/lib/lowdb')
}

const { Low, JSONFile } = low
const mongoDB = require('./funçoes/lib/mongoDB')
...
async function startclient() {
    const client = clientConnect({
        ...
        browser: ['Flay Search Multi Device','Safari','1.0.0'],
        auth: state
    })
    ...
    client.sendMessage(callerId, { text: `Bloqueio automático do sistema!\n\n❌ Proibido ligar para o Bot!\n\nContate a meu dono para ser desbloqueado!`}, { quoted: pa7rick })
    await sleep(8000)
    ...
    client.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./init")(client, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    // Get Profile Picture Group
       ...
       ppgc = 'https://shortlink.clientarridho.my.id/rg1oT'
       }
       let wm_fatih = { url : ppgc }
       if (pea[0].announce == true) {
  //     client.send5ButImg(pea[0].id, `「 Alteração nas configurações do grupo \n\nO grupo foi fechado pelo administrador, agora apenas o administrador pode enviar mensagens !`, `Mudança nas configurações do grupo`, wm_fatih, [])
